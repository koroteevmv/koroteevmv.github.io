...
![Классификация методов понижения размерности](/assets/images/ml_text/ml6-1.jpg){: .align-center style="width: 50%;"}
...
# Загружаем данные (4 признака, 150 образцов)
iris = load_iris()
X = iris.data          # матрица 150×4
y = iris.target
...
# Vt имеет форму (4,4); главные компоненты — строки Vt
components = Vt.T          # столбцы — главные компоненты
# Проекция на первые 2 компоненты
X_pca_manual = X_centered @ components[:, :2]

print("Форма проекции:", X_pca_manual.shape)
print("Первые 5 точек в новой системе координат:\n", X_pca_manual[:5])
```

```
Форма проекции: (150, 2)
Первые 5 точек в новой системе координат:
...
![PCA и регрессия](/assets/images/ml_text/ml6-2.png){: .align-center style="width: 90%;"}
...
Но метод главных компонент имеет нелинейную версию. Она иcпользует ядерные функции по аналогии с методом опорных векторов. Это позволяет делать проекции на нелинейные многообразия. Воспользуемся, например, радиально-базисной функцией:
...
Это уже приводит к совершенно другому преобразованию:
...
{% capture notice %}
**Определение (модель ICA).** В простейшем линейном варианте наблюдения $X$ (матрица $n \times m$) представляются как:

$$ X = S A^{\mathsf{T}} $$

где $S$ — матрица неизвестных независимых компонент (источников), а $A$ — неизвестная **матрица смешивания**. Задача ICA — найти **разделяющую матрицу** $W$, такую что $Y = X W$ даёт оценки независимых компонент.
{% endcapture %}
<div class="presentation">{{ notice | markdownify }}</div>

![PCA и ICA](/assets/images/ml_text/ml6-3.png){: .align-center style="width: 90%;"}

![PCA и ICA](/assets/images/ml_text/ml6-6.png){: .align-center style="width: 90%;"}

{% capture notice %}
Выводы:
1. Раскладывает датасет на независимые компоненты.
1. Применяется в обработке сигналов и изображений.
1. Не зависит от конкретного распределения данных.
1. Предполагает линейное смешивание компонентов.
1. Может быть вычислительно дорогим для больших датасетов.
{% endcapture %}
<div class="notice--info">{{ notice | markdownify }}</div>
...
![PCA и ICA](/assets/images/ml_text/ml6-4.webp){: .align-center style="width: 90%;"}

{% capture notice %}
В исходном пространстве точки строят карту сходства: для каждой точки мы смотрим на её ближайших соседей. Вероятность того, что $i$ выберет $j$ своим соседом, высока, если $j$ рядом, и падает экспоненциально с расстоянием.

В проекции мы тоже строим карту сходств, но используем t‑распределение с 1 степенью свободы. 

t‑SNE подстраивает координаты точек в проекции, чтобы сделать два распределения вероятностей похожими. Если в исходном пространстве $i$ и $j$ были соседями, в проекции они окажутся рядом. 
{% endcapture %}
<div class="presentation">{{ notice | markdownify }}</div>
...
![PCA и ICA](/assets/images/ml_text/ml6-5.gif){: .align-center style="width: 90%;"}

{% capture notice %}
Выводы:
1. В отличие от предыдущих методов, является стохастическим, результаты не детерминированы.
1. Чаще всего применяется для визуализации данных и описательного анализа данных.
1. Не сводится к переходу к иному базису как PCA или ICA.
1. По своей природе нелинеен, не требует введение ядерных функций.
1. Больше учитывает локальную структуру данных.
1. Благодаря тяжелым хвостам t-распределения, дальние точки могут разойтись еще дальше.
1. Расстояния между точками не являются интерпретируемыми.
1. Гораздо более вычислительно емкий, нежели PCA.
1. Не подходит для преобразования данных - трансдуктивный, а не индуктивный метод.
1. Может использоваться для поиска аномалий.
{% endcapture %}
<div class="notice--info">{{ notice | markdownify }}</div>
...
![Выбросы из тренда](/assets/images/ml_text/ml6-9.jpg){: .align-center style="width: 60%;"}
...
![Выбросы во временных рядах](/assets/images/ml_text/ml6-11.webp){: .align-center style="width: 60%;"}
...
![Множественный выбросы](/assets/images/ml_text/ml6-10.webp){: .align-center style="width: 60%;"}
...
![Гауссова аппроксимация](/assets/images/ml_text/ml6-8.png){: .align-center style="width: 60%;"}
...
![Локальный уровень выброса](/assets/images/ml_text/ml6-7.png){: .align-center style="width: 60%;"}
