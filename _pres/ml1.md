...
1. Примеры регрессионных задач - предсказание цены акции, оценка объекта недвижимости.
...
\frac{1}{2 m} \sum_{i=1}^{m} \Big( \frac{\partial}{\partial b_i} (h_b(x_i) - y_i)^2 \Big)
$$
{% endcapture %}
<div class="presentation">{{ notice-2 | markdownify }}</div>
...
$$
\frac{\partial}{\partial b_i} J = 
\frac{1}{m} \sum_{i=1}^{m} \Big( (h_b(x_i) - y_i) \cdot \frac{\partial}{\partial b_i} h_b(x_i) \Big)
$$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
\frac{1}{m} \sum \Big( (h_b(x_i) - y_i) \cdot x_i \Big)$$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
$$ \frac{\partial J}{\partial b_1} = 
\frac{1}{m} \sum \Big( (h_b(x_i) - y_i) \cdot x_i \Big) = 0$$
{% endcapture %}
<div class="presentation">{{ notice-2 | markdownify }}</div>
...
$$ b_1 := b_1 - \alpha \frac{1}{m} \sum_{i=1}^{m} \Big( (h_b(x_i) - y_i) \cdot x_i \Big) $$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
$$ h_b(x) = b_0 x_0 + b_1 x_1 + b_2 x_2 + ... + b_n x_n $$

Или в матричной форме:

$$ h\vec{y} = h_b(x) = X \cdot \vec{b} $$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
{% capture block %}
$$ J(\vec{b}) = \frac{1}{2m} (X \vec{b} - \vec{y})^T (X \vec{b} - \vec{y}) $$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
Если же подставить $x_0 = 1$, то все уравнения представляются в очень однообразной форме, которую в общем виде можно записать так:
...
{% capture block %}
$$ \vec{b} := \vec{b} - \frac{\alpha}{m} X^T (X \vec{b} - \vec{y}) $$
{% endcapture %}
<div class="presentation">{{ block | markdownify }}</div>
...
![Вытянутая функция ошибки 3D](/assets/images/ml_text/ml1-30.png "Вытянутая функция ошибки 3D"){: .align-center style="width: 50%;"}

Вот как это может выглядеть в виде контурного графика:

![Вытянутая функция ошибки](/assets/images/ml_text/ml1-31.png "Вытянутая функция ошибки"){: .align-center style="width: 50%;"}
